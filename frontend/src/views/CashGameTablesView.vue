<template>
  <div class="min-h-screen bg-slate-950">
    ...
    <div v-else-if="cashGame" id="cash-game-tables-view" class="bg-slate-950" style="padding: 0; margin: 0; min-height: 100vh; overflow: hidden; display: flex; flex-direction: column;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
          ...
          <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
            ...
            <div style="background: rgba(59, 130, 246, 0.8); border: 1px solid rgba(59, 130, 246, 0.4); color: white; padding: 0.375rem 0.75rem; border-radius: 6px; min-width: 75px; text-align: center;">
              <div style="font-size: 1.25rem; font-weight: 700; line-height: 1;">{{ stats.seated }}</div>
            ...
            <div style="background: rgba(16, 185, 129, 0.8); border: 1px solid rgba(16, 185, 129, 0.4); color: white; padding: 0.375rem 0.75rem; border-radius: 6px; min-width: 75px; text-align: center;">
              <div style="font-size: 1.25rem; font-weight: 700; line-height: 1;">{{ stats.playing }}</div>
            ...
            <div style="background: rgba(251, 191, 36, 0.8); border: 1px solid rgba(251, 191, 36, 0.4); color: white; padding: 0.375rem 0.75rem; border-radius: 6px; min-width: 95px; text-align: center;">
              <div style="font-size: 1.25rem; font-weight: 700; line-height: 1;">₾{{ formatNumber(stats.total_pot || 0) }}</div>
              <div style="font-size: 0.625rem; opacity: 0.9; margin-top: 0.0625rem;">Total Pot</div>
            </div>
          </div>
          
          <!-- Right: Fullscreen & Back Button -->
      ...
      <div style="flex: 1; display: flex; gap: 1rem; padding: 1rem; min-height: 0; overflow-y: auto; background: #0f172a;">
        ...
        <div style="flex: 1; display: flex; flex-direction: column; min-height: 0;">
          <div class="table-card card" style="overflow: hidden; height: 100%;">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                  ...
                  <div>
                    ...
                    <div style="font-size: 0.75rem; opacity: 0.9; margin-top: 0.125rem;">
                      {{ getPlayingCount() }}/{{ table.total_seats }} Playing
                    </div>
                  </div>
                </div>
                <div :style="getTableStatusStyle()">
                  ...
                  >
                    <template v-if="seat.occupied">
                      ...
                      <div :style="getSeatStyle(seat)">
                        ...
                        <div v-if="seat.current_stack > 0" style="color: white; font-size: 0.75rem; margin-top: 0.25rem; opacity: 0.9;">
                          ₾{{ seat.current_stack.toLocaleString() }}
                        </div>
                      </div>
                    </template>
                    <template v-else>
            ...
            <div style="background: #1e293b; padding: 0.75rem 1rem; border-top: 1px solid #334155; display: flex; align-items: center; justify-content: space-between; font-size: 0.875rem;">
              <div style="display: flex; gap: 1rem;">
                <div style="display: flex; align-items: center; gap: 0.375rem;">
                  ...
                  <span style="color: #cbd5e1;"><strong style="color: #f1f5f9;">{{ getPlayingCount() }}</strong> Playing</span>
                </div>
                <div style="display: flex; align-items: center; gap: 0.375rem;">
                  <div style="width: 10px; height: 10px; background: #3b82f6; border-radius: 50%; box-shadow: 0 0 4px rgba(59, 130, 246, 0.5);"></div>
                  <span style="color: #cbd5e1;"><strong style="color: #f1f5f9;">{{ getSeatedCount() }}</strong> Seated</span>
                </div>
                <div style="display: flex; align-items: center; gap: 0.375rem;">
                  <div style="width: 10px; height: 10px; background: #64748b; border-radius: 50%;"></div>
                  <span style="color: #cbd5e1;"><strong style="color: #f1f5f9;">{{ getEmptyCount() }}</strong> Empty</span>
                </div>
              </div>
            ...
            <div style="flex: 1; overflow-y: auto; padding: 0.75rem; background: #1e293b;">
              <template v-if="waitingList.length > 0">
                <div 
                  ...
                  @mouseover="$event.target.style.background='rgba(245, 158, 11, 0.15)'; $event.target.style.borderColor='rgba(245, 158, 11, 0.4)'"
                  @mouseout="$event.target.style.background='rgba(245, 158, 11, 0.1)'; $event.target.style.borderColor='rgba(245, 158, 11, 0.3)'"
                >
...
const cashGameId = route.params.id
...
const fetchCashGameData = async () => {
  try {
    isLoading.value = true
    
    // Fetch cash game details
    const cashGameRes = await axios.get(`/cash-games/${cashGameId}`)
    cashGame.value = cashGameRes.data.cash_game || cashGameRes.data
    
    // Fetch table layout
    const tablesRes = await axios.get(`/cash-games/${cashGameId}/tables`)
    table.value = {
      ...tablesRes.data.table,
      total_seats: cashGame.value.seats_per_table || tablesRes.data.table.total_seats || 0
    }
    
    // Fetch waiting list
    try {
      const waitingRes = await axios.get(`/cash-games/${cashGameId}/waiting-list`)
    ...
    // Calculate stats
    const totalSeats = cashGame.value.seats_per_table
    const occupied = table.value.occupied_count
    const available = Math.max(0, totalSeats - occupied)
    const fillPercentage = totalSeats > 0 ? Math.round((occupied / totalSeats) * 100) : 0
    
    const seated = table.value.seats.filter(s => s.occupied && (s.status === 'seated' || s.status === 'away')).length
    const playing = table.value.seats.filter(s => s.occupied && s.status === 'playing').length
    
    // Calculate total pot from current stacks
    const totalPot = table.value.seats
      .filter(s => s.occupied && s.current_stack > 0)
      .reduce((sum, s) => {
        const stack = parseFloat(s.current_stack) || 0
        return sum + stack
      }, 0)
    ...
    console.error('Error fetching cash game data:', error)
  } finally {
...
const getPlayingCount = () => {
  return table.value.seats.filter(s => s.occupied && s.status === 'playing').length
}

const getSeatedCount = () => {
  return table.value.seats.filter(s => s.occupied && (s.status === 'seated' || s.status === 'away')).length
}

const getEmptyCount = () => {
  return table.value.seats.filter(s => !s.occupied).length
}

const getTableStatusStyle = () => {
  const isFull = table.value.occupied_count >= table.value.total_seats
  const isAlmostFull = table.value.occupied_count >= table.value.total_seats * 0.8
...
const getSeatStyle = (seat) => {
  ...
  } else if (seat.status === 'sitting_out') {
...
const toggleFullscreen = () => {
  const elem = document.getElementById('cash-game-tables-view')
